...
export function showTitleScreen({ onNew, onContinue }) {
  ...
  let raf = 0, consuming = false, finished = false, startT = 0;
  const GROW_MS = 2600;   // left-to-right creep duration
  ...
  function newGrowth() {
    ...
    return {
      ...
      frontX: Infinity, minX: 0, maxX: 0, pendingSeeds: [],   // left-to-right growth front (see frame())
    };
  }
  ...
  function step(gr) {
    ...
    for (const at of gr.attractors) {
      if (at.x > gr.frontX) { survivors.push(at); continue; }   // ahead of the growth front — not yet
  ...
  function seedTitle() {
    ...
    const seeds = pts.splice(0, Math.min(220, pts.length));   // added left-to-right by the front (frame())
    g.attractors = pts;
    ...
    const edge = fringe.length ? fringe : seeds;
    ...
    // Growth front: seeds + attractors activate left-to-right so the word creeps in.
    let minX = Infinity, maxX = -Infinity;
    for (const a of g.attractors) { if (a.x < minX) minX = a.x; if (a.x > maxX) maxX = a.x; }
    for (const s of seeds) { if (s.x < minX) minX = s.x; if (s.x > maxX) maxX = s.x; }
    g.minX = minX; g.maxX = maxX;
    g.pendingSeeds = seeds.sort((a, b) => a.x - b.x);
    g.frontX = minX - 1;
    if (reduce) {                                             // no animation — grow to completion at once
      for (const s of seeds) addNode(g, s.x, s.y, -1);
      g.pendingSeeds = []; g.frontX = maxX + 1e9;
      let guard = 0; while (!g.done && guard++ < 40000) step(g); flushInk();
    }
  }
  ...
  function frame(t) {
    if (finished) return;
    if (!startT) startT = t || 1;
    if (!consuming) {                                   // advance the left-to-right growth front
      const prog = Math.min(1, ((t || startT) - startT) / GROW_MS);
      g.frontX = g.minX + (g.maxX - g.minX + 6) * prog;
      while (g.pendingSeeds.length && g.pendingSeeds[0].x <= g.frontX) { const s = g.pendingSeeds.shift(); addNode(g, s.x, s.y, -1); }
    }
    if (!g.done || consuming) { for (let i = 0; i < 3; i++) step(g); flushInk(); }
    composite();
  ...
  function consume(btn, cb) {
    ...
    g.done = false; g.frontX = 1e9;          // consume ignores the growth front (grow anywhere now)
  ...
  function layout() {
    ...
    startT = 0;             // restart the creep-in animation for the new layout
    seedTitle();
    composite();
  }
