...
export function showTitleScreen({ onNew, onContinue }) {
  ...
  function newGrowth() {
    const seg = Math.max(2.2, titleSize * 0.024);
    const attract = titleSize * 0.28;
    return {
      nodes: [], segs: [], attractors: [], map: new Map(),
      seg, attract, attract2: attract * attract, kill2: (seg * 1.7) ** 2, cell: attract, _bd: 0, done: false,
    };
  }

  function step(gr) {
    if (!gr.attractors.length || gr.nodes.length > 16000) { gr.done = true; return; }
  ...
  function seedTitle() {
    g = newGrowth();
    // faint "core glow" so the word always reads, even where filaments are sparse
    ictx.save();
    ictx.filter = 'blur(' + (titleSize * 0.02 * dpr) + 'px)';
    ictx.fillStyle = 'rgba(150,220,190,0.13)';
    ictx.textAlign = 'center'; ictx.textBaseline = 'middle';
    ictx.font = titleFont(titleSize * dpr);
    ictx.fillText(TITLE, cx * dpr, titleY * dpr);
    ictx.restore();
    // glyph attractors (dense) → filaments fill the letters
    ...
    }, Math.max(3, g.seg * 1.1));
    // seeds: a spread of glyph points become the first nodes (many → every letter covered)
    for (let i = pts.length - 1; i > 0; i--) { const j = (Math.random() * (i + 1)) | 0; const t = pts[i]; pts[i] = pts[j]; pts[j] = t; }
    const seeds = pts.splice(0, Math.min(40, pts.length));
    for (const s of seeds) addNode(g, s.x, s.y, -1);
    g.attractors = pts;
    // outward halo — sparse points ringing the word so tendrils creep outward
    const m = ctx; m.font = titleFont(titleSize);
    const tw = m.measureText(TITLE).width, th = titleSize;
    for (let i = 0; i < 210; i++) {
      const a = rnd(0, Math.PI * 2), rx = tw * 0.6 * (1 + rnd(0.05, 0.6)), ry = th * 0.8 * (1 + rnd(0.1, 1.1));
      g.attractors.push({ x: cx + Math.cos(a) * rx, y: titleY + Math.sin(a) * ry });
    }
    if (reduce) { let guard = 0; while (!g.done && guard++ < 12000) step(g); flushInk(); }
  }

  // ---- rendering ---------------------------------------------------------
  function flushInk() {
    if (!g.segs.length) return;
    ictx.strokeStyle = 'rgba(233,255,247,0.92)';
    ictx.lineWidth = 1.15 * dpr; ictx.lineCap = 'round';
    ictx.beginPath();
