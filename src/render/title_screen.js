...
export function showTitleScreen({ onNew, onContinue }) {
  ...
  function seedTitle() {
    ...
    // short stray "hairs" sprouting in random directions from a few edge points
    for (let i = 0; i < 45; i++) {
      const s = seeds[(Math.random() * seeds.length) | 0]; if (!s) break;
      const ang = rnd(0, Math.PI * 2), n = 2 + ((Math.random() * 3) | 0);
      for (let k = 1; k <= n; k++) g.attractors.push({ x: s.x + Math.cos(ang) * g.attract * 0.55 * k, y: s.y + Math.sin(ang) * g.attract * 0.55 * k });
    }
    // long outward tendrils: chains of attractors reaching out in all directions
    for (let i = 0; i < 28; i++) {
      const s = seeds[(Math.random() * seeds.length) | 0]; if (!s) break;
      const ang = Math.atan2(s.y - titleY, s.x - cx) + rnd(-0.6, 0.6);
