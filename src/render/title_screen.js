...
export function showTitleScreen({ onNew, onContinue }) {
  ...
  const CONSUME_RATE = 12;  // the menu word grows at a finer scale (many more strands) — step fast so it completes in time
  ...
  function bridgeAttractors(ax, ay, bx2, by2) {
    ...
    for (let k = 1; k <= n; k++) {
      ...
      if (k > 1 && k < n && Math.random() < 0.5) {               // side branch with a tip spray
  ...
  function sampleWord(btn) {
    ...
    const S = 3;   // supersample factor → ~3× finer effective sampling than the 1px-integer floor
    const draw = (o, blur) => {
    ...
    const SPL = 3;   // bloom seeds per letter → several fans fill each glyph densely
    m.font = font; try { m.letterSpacing = spacing; } catch (_) {}
    const x0 = bx - total / 2, seeds = []; let prevW = 0;
    for (let i = 0; i < text.length; i++) {
      ...
      for (let s = 0; s < SPL && inL.length; s++) seeds.push(inL[(Math.random() * inL.length) | 0]);
    }
    try { m.letterSpacing = '0px'; } catch (_) {}
    return { glyph, fringe, seeds, bx, by, size };
  }

  // Grow the button's word so it reads exactly like MYCELIUM: growth switches to the
  // WORD's OWN (finer) scale so a small word gets as many strands, relatively, as the
  // big title; EVERY letter blooms from its own random points with dense glyph
  // attractors + hairy strays; AND 1–2 branching connector strands creep out of the
  // nearest MYCELIUM letters so the word visibly grows straight out of the title.
  function growButtonWord(btn) {
    const { glyph, fringe, seeds, bx, by, size } = sampleWord(btn);
    if (!seeds.length) { g.done = false; return; }
    // rescale growth to the word (leave g.cell alone — it keeps the title's grid buckets
    // valid; a smaller attract than cell just makes nearestNode search a superset, fine)
    g.seg = Math.max(0.6, size * 0.009);
    g.attract = size * 0.05; g.attract2 = g.attract * g.attract; g.kill2 = g.seg * g.seg;
    for (const p of glyph) g.attractors.push(p);
    for (const f of fringe) g.attractors.push(f);
    // (A) immediately: 1–2 branching connector strands out of the nearest MYCELIUM nodes.
    ...
    g.done = false;
    // (B) a beat later: every letter blooms from its own seeds (dense + legible) + strays.
    const bloom = () => {
      if (finished) return;
      for (const s of seeds) addNode(g, s.x, s.y, -1);
      const edge = fringe.length ? fringe : glyph, sp = g.attract * 0.6, strays = Math.round(seeds.length * 10);
  ...
  function consume(btn, cb) {
    ...
    setTimeout(finishFn, 2700);                      // (3) fade to the next screen a beat after the word forms
  }
