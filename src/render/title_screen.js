...
export function showTitleScreen({ onNew, onContinue }) {
  ...
  function seedTitle() {
    ...
    g.attractors = pts;
    // ONE seed per letter — each letter grows outward from a single central point.
    ...
    for (let i = 0; i < TITLE.length; i++) {
      ...
      const bcx = x0 + (prevW + w) / 2; prevW = w;                 // this letter's x-centre
      let best = null, bd = Infinity;                             // nearest glyph point to the letter centre
      for (const pt of pts) { const d = (pt.x - bcx) ** 2 + (pt.y - titleY) ** 2; if (d < bd) { bd = d; best = pt; } }
      if (best) addNode(g, best.x, best.y, -1);
    }
  ...
  function frame() {
    if (finished) return;
    if (!g.done || consuming) { for (let i = 0; i < 2; i++) step(g); flushInk(); }   // ~2 steps/frame paces the bloom
    composite();
