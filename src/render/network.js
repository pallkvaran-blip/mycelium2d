...
export class NetworkRenderer {
  ...
  redrawBright(ctx, camera, time, brightness = 1) {
    ...
    // Subtle mint SHEEN to echo the lit colony's glow: additive, stroked ALONG the strands
    // (a touch wider for a soft edge) so it brightens the colony toward mint-white without a
    // radial halo bleeding into the earth.
    const featherW = 1.4 / Math.max(0.001, zoom);   // ~0.7px each side — hugs the strand
