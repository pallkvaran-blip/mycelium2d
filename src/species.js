...
// EXTRA nematodes + Trichoderma piled on top of the base curve, so a good engine stops
// trivialising the mid/late ladder (owner call after a player cleared to 20 unopposed and
// quit out of boredom). Ants are deliberately excluded — they keep their own capped curve.
//   L1–6 → +0   (the on-ramp is unchanged)
//   L7–9 → +2, L10–14 → +3, then +1 more every 5 levels from 15:
//   L15 +4, L20 +5, L25 +6, L30 +7, … L100 +21.
// So the seeded count is level + bonus for each: L7 → 9, L10 → 13, L20 → 25, L30 → 37,
// L100 → 121 (still under `config.nematodes.maxPopulation` = 150, so seeding can't blow
// the hard cap — but note it leaves little breeding headroom that deep).
