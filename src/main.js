...
function drawMountains() {
  ...
  runs.forEach((run, i) => {
    ...
    // Fainter overall (backdrop-like); larger peaks stay a touch more present.
    const t = (run.wCells - MOUNTAIN_W_MIN) / (MOUNTAIN_W_MAX - MOUNTAIN_W_MIN);
    ctx.save();
    ...
    ctx.clip();
    ctx.globalAlpha = 0.5 + 0.3 * Math.max(0, Math.min(1, t));
    ctx.drawImage(img, s.x - sw / 2, by - sh, sw, sh);
    ctx.restore();
  });
}
