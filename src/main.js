...
function drawCloudSight() {
  ...
  ctx.save();
  // A THIN faint outline marks the range. It used to be a wide soft radial rim,
  // but the sight radius is large (≈500 world units), so on screen that rim was a
  // big smooth gradient band that read as an out-of-place "smooth brown" blob over
  // the earth — most obvious under cities, where no rock covers it. A constant
  // thin line gives the same range cue without the gradient.
  ctx.lineWidth = 1.25;
  for (const c of clouds) {
    const s = camera.worldToScreen(c.cx, c.cy);
    ctx.strokeStyle = `rgba(150,190,70,${c.dying ? 0.10 : 0.16})`;
