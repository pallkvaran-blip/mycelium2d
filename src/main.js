...
function drawLakes() {
  ...
  for (const run of runs) {
    ...
    // CLIP the art to the actual water-cell BOWL (a lake is a semi-ellipse — deep at the
    // centre, shallow at the edges — but the art image's own bowl can be fuller/narrower).
    // Clipping to the real cells makes the visible pool == the water the sim uses, so a
    // strand hugging the cells reads as touching the pool's edge (not poking into empty
    // teal, and no phantom water over dry soil). Bottom follows each column's water depth.
    ...
    for (let col = run.c1; col >= run.c0; col--) {                // bottom bowl, right → left
      let d = 0; while (d < sub.rows && sub.cellAt(col, d) && sub.cellAt(col, d).water) d++;
      const xc = camera.worldToScreen((col + 0.5) * cs, sub.surfaceY).x;
      const yb = camera.worldToScreen(0, sub.surfaceY + d * cs).y;
      ctx.lineTo(xc, yb);
    }
    ctx.closePath();
    ctx.clip();
