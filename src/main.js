...
const CITY_W_MIN = 16, CITY_W_MAX = 26;   // city width in tiles (~20), slight per-city variation
...
function cityRuns() {
  ...
  while (c < sub.cols) {
    if (sub.surface[c] && sub.surface[c].barrier === 'concrete') {
      ...
      const wCells = CITY_W_MIN + _hashf(c + 0.3, (state.seed || 1) * 0.029) * (CITY_W_MAX - CITY_W_MIN);
      runs.push({ c0: c, c1: end - 1, wCells });
      c = end;
    } else c++;
  }
