...
function drawWallFormations() {
  ...
  walls.forEach((g, wi) => {
    ...
    // dark core fill behind the stack — the impassable core never shows earth
    const t0 = camera.worldToScreen(minX - cs / 2, topY);
    const t1 = camera.worldToScreen(maxX + cs / 2, botY);
    ctx.save();
