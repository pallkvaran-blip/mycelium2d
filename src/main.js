...
function drawGoalBackdrop() {
  ...
  const embed = cs * 0.9;                                             // sink the grassy base so the hill meets the soil line flush
  ...
  withWorldClip(() => ctx.drawImage(hill, tl.x, tl.y, sw, sh));
}
function drawGoalProps() {
  ...
  withWorldClip(() => {
    for (const [col, hCells] of [[g0 + 1, 3.0], [g0 + 4, 2.2]]) {    // a big cluster + a smaller one
      ...
      const s = camera.worldToScreen((col + 0.5) * cs, sub.surfaceY + cs * 0.6);   // base sunk into the soil so it sits flush
      const sw = wWorld * z, sh = hWorld * z;
