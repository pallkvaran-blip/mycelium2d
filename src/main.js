  ...
  onPlayCard(index) {
    ...
    if (cardNeedsTarget(entry.name)) {
      ...
      ui.setHint(cardUsesDragAim(entry.name)
        ? `Press on your colony and drag to aim ${entry.name} — release to grow there, or drag far to cancel.`
  ...
  onActivateAction(i) {
    ...
    if (res && res.needTarget) {
      ...
      ui.setHint(act && act.aim === 'drag'
        ? `Press on your colony and drag to aim ${act.name} — release to grow there, or drag far to cancel.`
        : res.message);
      uiDirty = true;
      return;
    }
...
// Mark the soil cells under one rock sprite's opaque silhouette as solid rock.
// The sprite is drawn centred at (cxW,cyW) in world units, size (wW×hW), rotated
// by `rot` (matching the draw). Returns false if the sprite isn't decodable yet.
function stampSolid(sub, cs, img, cxW, cyW, wW, hW, rot) {
  ...
  for (let col = c0; col <= c1; col++) {
    for (let row = r0; row <= r1; row++) {
      ...
      if (cell.nutrient > 0 || cell.maxNutrient > 0) continue;   // don't bury a food pile
      const ctr = sub.cellCenter(col, row);
      ...
      cell.rock = true; cell.rockFill = true; cell.hazard = false;
    }
  }
  return true;
}

// One-shot per map: solidify the visible silhouette of EVERY rock sprite —
// scattered boulders, big formations, and vertical columns — so no rock type can
// be grown over. (Winnability is not protected here by design.)
function solidifyRock() {
  ...
  for (const g of rockGroups()) {
    ...
    for (const c of g.cells) {
      ...
      if (!stampSolid(sub, cs, img, c.x + ox, c.y + oy, bwW, bhW, (h3 * 2 - 1) * 0.3)) ready = false;
    }
  }
  ...
  for (const g of formationGroups()) {
    ...
    if (!stampSolid(sub, cs, r.img, r.cx, (r.topW + r.baseY) / 2, r.dw, r.baseY - r.topW, 0)) ready = false;
  }
  ...
  if (cols && cols.length && ROCKFORM_KEYS.some(hasAsset)) {
    ...
    cols.forEach((col, ci) => {
      ...
      for (let i = 0; i < N; i++) {
        ...
        if (!stampSolid(sub, cs, img, startX + ux * t, startY + uy * t, longW, crossW, rot)) ready = false;
      }
    });
  }

  if (ready) sub._rockSolidified = true;
}
