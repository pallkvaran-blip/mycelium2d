...
// Drop a 1⚡ nut cache at ctx, gated by colony LINE OF SIGHT — a clear straight line
// from ANY living strand (rock blocks it; NO distance cap). Snaps to the nearest open
// soil. Returns {ok,message} on success, or {ok:false,message,losHint} when nothing has
// a clear line (losHint tells the UI to offer the "Colony line of sight" overlay).
// Shared by the Decoy Cache event and the installed Perennial Decoy action.
function dropDecoyCache(s, ctx) {
...
EFFECTS['Perennial Decoy'] = action({ effect: 'drop a 1⚡ food cache in sight', every: 8, cost: 1, res: 'water', target: true }, (s, ctx) => dropDecoyCache(s, ctx));
