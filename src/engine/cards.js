...
// --- setup ------------------------------------------------------------------
// Build the starting deck+hand. `mode`: 'tutorial' (default) seeds the tutorial
// hand; 'all' puts every premium card in hand (dev / free explore).
export function initCards(state, mode = 'tutorial') {
  ...
  for (const c of CARD_DATA) {
    ...
    const take = mode === 'all' ? true : c.tutorial;
    if (take) hand.push({ id: seq++, name: c.name });
  }

  state.cards = { drawDeck, hand, discard: [], engines: [], round: 1, seq, drawDiscount: 0 };
  state.log('Card layer online: draw basics, play from your hand, reach the goal.', 'good');
  return state.cards;
}
