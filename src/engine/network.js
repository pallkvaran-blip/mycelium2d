...
export class Network {
  ...
  punchThrough(substrate, rng, x, y) {
    ...
    // How far the rock reaches along the ray (from the anchor). Used to bore the
    // channel and to bound the strand so it stops where the rock ends.
    let far = 0;
    ...
    // Thread a straight strand from the anchor, through the rock, and STOP where
    // the rock ends — grow only up to the rock's far edge (+ a hair so the tip
    // clears it). It must NOT run on through the open ground beyond (that made the
    // card OP); food sitting right on the far side is picked up by the colonisation
    // pass below, not by over-growing. Distance-capped so a wide/irregular rock is
    // fully crossed without stopping early on an internal gap.
    const stopDist = far + substrate.cellSize * 0.75;
