// =============================================================================
// Actions — the seven basic actions, the backbone of the game (3D port).
//
// Each action is defined as tunable DATA (moveCost / energyCost in CONFIG) plus
// an effect. performAction() handles the energy economy so every action is
// uniform, and advances the whole world one step on success.
//
// Targeting:
//   - 'point' : effect needs a world {x, y, z} (Add Substrate, Attack Ants)
//   - 'node'  : effect needs a world {x, y, z} to pick strands (Amputate)
//   - null    : no target (Grow, Excrete, Digest, Fruit)
// =============================================================================
...
export const ACTIONS = {
  ...
  addSubstrate: {
    ...
    apply(state, ctx) {
      ...
      if (ctx.y >= 0) return { ok: false, message: 'Place substrate underground (below the surface).' };
      const cell = sub.cellAtWorld(ctx.x, ctx.y, ctx.z);
      if (cell && cell.rock) return { ok: false, message: 'Can’t place substrate inside rock.' };
      sub.deposit(ctx.x, ctx.y, ctx.z, a.amount, a.radius);   // small amount — it lures, it doesn't feed
  ...
  amputate: {
    ...
    apply(state, ctx) {
      ...
      const removed = state.active.amputateAt(ctx.x, ctx.y, ctx.z, a.radius);
  ...
  attackAnts: {
    ...
    apply(state, ctx) {
      ...
      const hit = attackNest(state, ctx.x, ctx.y, ctx.z, a.pickRadius, a.damageFrac);
  ...
  fruit: {
    ...
    apply(state) {
      ...
      if (points.length === 0) {
        return { ok: false, message: 'No fruitable soil reached — steer growth under the goal soil near the surface.' };
      }
...
// What it costs to perform an action right now.
...
// --- Dev cheat: spawn a Trichoderma cloud at a point --------------------------
export function devSpawnTrichoderma(state, x, y, z) {
  spawnTrichodermaAt(state, x, y, z);
