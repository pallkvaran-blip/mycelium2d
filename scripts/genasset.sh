#!/usr/bin/env bash
# Generate a tileable texture via Replicate (FLUX) and save it into assets/.
#
#   REPLICATE_API_TOKEN=... scripts/genasset.sh <key> "<prompt>" [aspect] [model]
#
# Writes assets/<key>.png. Requires REPLICATE_API_TOKEN (never store in the repo).
...
body=$(python3 -c 'import json,sys; print(json.dumps({"input":{"prompt":sys.argv[1],"aspect_ratio":sys.argv[2],"output_format":"png","num_outputs":1}}))' "$PROMPT" "$ASPECT")
...
for i in 1 2 3 4 5 6 7 8; do
  ...
  if printf '%s' "$resp" | grep -q '"status":429'; then
    ...
    echo "throttled, waiting $((ra + 2))s..."; sleep $((ra + 2)); continue
  fi
  break
done
...
EOF
if [ -z "${url:-}" ]; then echo "no image (status=${status:-?}):"; printf '%s' "$resp" | head -c 800; exit 1; fi
