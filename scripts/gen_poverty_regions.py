...
def build():
    ...
    fig.subplots_adjust(left=0.075, right=0.755, top=0.775, bottom=0.115)
    ...
    ax.grid(True, axis="y", color="#ffffff", lw=1.0, ls="-", zorder=6, alpha=0.30)
    for side in ("top", "right"): ax.spines[side].set_visible(False)
    for side in ("left", "bottom"):
        ax.spines[side].set_color(AXIS); ax.spines[side].set_linewidth(1.0)

    # --- in-band labels for the three bands thick enough to hold them
    for nm, xpos, ypos, col in [("East Asia & Pacific", 1997.0, 1050, "#ffffff"),
                                ("South Asia", 2001.5, 1620, "#ffffff"),
                                ("Sub-Saharan Africa", 2032.0, 430, "#ffffff")]:
        ax.text(xpos, ypos, nm, fontsize=14, fontweight="bold", color=col,
    ...
    # --- legend: every series named, which is also the relief the validator asks for
    handles = [Patch(facecolor=c, edgecolor="none", label=n)
               for n, c in zip(names[::-1], cols[::-1])]
    ax.legend(handles=handles, loc="upper left", bbox_to_anchor=(1.012, 1.0),
              frameon=False, fontsize=12.5, labelspacing=0.95, handlelength=1.1,
              handleheight=1.1, borderpad=0, labelcolor=TEXT_2)
    ...
    for ext in ("png", "svg"):
        fig.savefig(OUT / f"poverty-regions.{ext}", format=ext)
    plt.close(fig)
    for yr in (1990, 2020, 2030, 2040):
        print(f"  {yr}: total {total[np.argmin(abs(xs-yr))]:.0f}m")
    print("wrote poverty-regions.png / .svg")
