#!/usr/bin/env python3
# Artist's Conk (Ganoderma applanatum) portrait options -> assets/species_options/.
...
LOG = os.environ.get("GCLOG", "/tmp/ganoderma.log")
MODEL = "black-forest-labs/flux-1.1-pro"
NEG = ("candid amateur field photograph, shot on a DSLR with a 100mm macro lens, natural "
       "overcast daylight, true-to-life muted colours, realistic imperfections, specks of soil "
       "and bark debris, matte damp surface, subtle grain, realistic depth of field, documentary "
       "mushroom field-guide photo, unretouched, not glossy, not CGI, not rendered, no text, no watermark, no border")
PROMPT = ("Ganoderma applanatum, the artist's conk: a large hard woody semicircular shelf bracket "
          "growing from the trunk of a dead hardwood tree, a flat concentrically-zoned and furrowed "
          "grey-brown upper surface dusted with cocoa-brown spore powder, a chalk-white pore underside, "
          "layered growth, damp shaded forest, " + NEG)
def log(m):
...
def main():
    ...
    for n in (1,2,3): gen(n, seed=6100+n*167); time.sleep(1)
    log("GANODERMA DONE")
